...
if (-not (Test-Path $environmentPath)) {
    ...
    $existingEnvironment = Read-DotEnv $environmentPath
    $existingApiKey = $existingEnvironment["OPENROUTER_API_KEY"]
    try {
        $normalizedApiKey = Assert-OpenRouterApiKey $existingApiKey
        if ($normalizedApiKey -cne $existingApiKey) {
            Set-SamratOpenRouterApiKey -EnvironmentPath $environmentPath -OpenRouterApiKey $normalizedApiKey | Out-Null
            Write-Host "Cleaned the existing OpenRouter API key."
        } else {
            Write-Host "Existing configuration found; keeping its database and keys."
        }
    } catch {
        Write-Host "The saved OpenRouter API key is invalid and must be replaced." -ForegroundColor Yellow
        $secureApiKey = Read-Host "Paste the OpenRouter API key again (copy only the key value)" -AsSecureString
        $apiKey = ConvertFrom-SecureValue $secureApiKey
        try {
        ...
        Write-Host "Replaced the invalid OpenRouter API key." -ForegroundColor Green
    }
}
