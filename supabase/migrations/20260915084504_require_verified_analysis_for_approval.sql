...
begin
 ...
 if p_action in ('accept','reject') then
   ...
   if p_action='accept' and (
     coalesce(c.processing_meta #>> '{analysisIntegrity,version}','') <> '1' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,status}','') <> 'verified' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,extractionSucceeded}','') <> 'true' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,authoritativeReviewCompleted}','') <> 'true' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,documentCount}','0') = '0' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,auditedDocumentCount}','') <> coalesce(c.processing_meta #>> '{analysisIntegrity,documentCount}','') or
     coalesce(c.processing_meta #>> '{analysisIntegrity,assessedPageCount}','0') = '0' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,unsafePageCount}','') <> '0' or
     coalesce(c.processing_meta #>> '{analysisIntegrity,unresolvedDocumentCount}','') <> '0' or
     coalesce(c.processing_meta #>> '{extractionReview,enabled}','') <> 'true' or
     coalesce(c.processing_meta #>> '{extractionReview,required}','') <> 'true' or
     coalesce(c.processing_meta #>> '{extractionReview,authoritative}','') <> 'true' or
     coalesce(c.processing_meta #>> '{lastProcessingError}','') <> ''
   ) then raise exception 'Approval is blocked because document reading and extraction were not fully verified. Run the analysis again before approval.'; end if;
...
declare c public.packet_cases; result jsonb; next_status text; analysis_safe boolean;
begin
 ...
 analysis_safe=(
   coalesce(c.processing_meta #>> '{analysisIntegrity,version}','') = '1' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,status}','') = 'verified' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,extractionSucceeded}','') = 'true' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,authoritativeReviewCompleted}','') = 'true' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,documentCount}','0') <> '0' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,auditedDocumentCount}','') = coalesce(c.processing_meta #>> '{analysisIntegrity,documentCount}','') and
   coalesce(c.processing_meta #>> '{analysisIntegrity,assessedPageCount}','0') <> '0' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,unsafePageCount}','') = '0' and
   coalesce(c.processing_meta #>> '{analysisIntegrity,unresolvedDocumentCount}','') = '0' and
   coalesce(c.processing_meta #>> '{extractionReview,enabled}','') = 'true' and
   coalesce(c.processing_meta #>> '{extractionReview,required}','') = 'true' and
   coalesce(c.processing_meta #>> '{extractionReview,authoritative}','') = 'true' and
   coalesce(c.processing_meta #>> '{lastProcessingError}','') = ''
 );
 next_status=case
   when exists(select 1 from public.packet_mismatches where case_id=p_case and field_name='documentReadability') then 'completed'
   when analysis_safe and not exists(select 1 from public.packet_mismatches where case_id=p_case and resolution_status<>'accepted') then 'accepted'
   else 'completed'
 end;
