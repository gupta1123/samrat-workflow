...
function input(overrides: Record<string, unknown> = {}) {
  return {
    ...
    invoiceLines: [
      { itemCode: "VIV10796", description: "Steel Coil", quantity: 2 },
    ],
...
test("builds a Test AP Invoice Draft based on a verified open GRPO line", () => {
  ...
  assert.deepEqual(draft.DocumentLines, [
    { BaseType: 20, BaseEntry: 8574, BaseLine: 0, Quantity: 2 },
  ]);
});
...
test("builds the same SAP Test draft directly from a verified open purchase order", () => {
  ...
  assert.deepEqual(draft.DocumentLines, [
    { BaseType: 22, BaseEntry: 8574, BaseLine: 0, Quantity: 2 },
  ]);
});
...
test("selects the correct open PO line when an item code is repeated at different quantities", () => {
  const draft = buildPoApInvoiceDraft({
    ...input({
      invoiceLines: [
        { itemCode: "DM0008", description: "Glow Sigin Board", quantity: 192 },
      ],
    }),
    po: {
      ...grpo,
      DocumentLines: [
        {
          LineNum: 0,
          ItemCode: "DM0008",
          ItemDescription: "Glow Sigin Board",
          RemainingOpenQuantity: 192,
          LineStatus: "bost_Open",
        },
        {
          LineNum: 1,
  ...
  assert.deepEqual(draft.DocumentLines, [
    { BaseType: 22, BaseEntry: 8574, BaseLine: 0, Quantity: 192 },
  ]);
});

test("selects separate PO lines by rate when item, description, and quantity are identical", () => {
  const draft = buildPoApInvoiceDraft({
    ...input({
      invoiceLines: [
        {
  ...
  assert.deepEqual(draft.DocumentLines, [
    { BaseType: 22, BaseEntry: 8574, BaseLine: 1, Quantity: 450 },
    { BaseType: 22, BaseEntry: 8574, BaseLine: 0, Quantity: 450 },
  ]);
});
...
test("rejects a closed or mismatched purchase order before any SAP write", () => {
  assert.throws(
    () =>
      buildPoApInvoiceDraft({
        ...input(),
        po: { ...grpo, DocumentStatus: "bost_Close" },
      }),
    /no longer open/,
  );
  assert.throws(
    () =>
      buildPoApInvoiceDraft({ ...input(), po: { ...grpo, CardCode: "OTHER" } }),
    /identity/,
  );
});
...
test("rejects wrong GRPO or vendor before a SAP write", () => {
  assert.throws(
    () => buildApInvoiceDraft(input({ expectedDocEntry: 8575 })),
    /identity/,
  );
  assert.throws(
    () => buildApInvoiceDraft(input({ invoiceVendor: "OTHER SUPPLIER" })),
    /vendor/,
  );
});
...
test("rejects unmatched and over-quantity invoice lines", () => {
  assert.throws(
    () =>
      buildApInvoiceDraft(
        input({ invoiceLines: [{ description: "Copper Pipe", quantity: 2 }] }),
      ),
    /exactly match/,
  );
  assert.throws(
    () =>
      buildApInvoiceDraft(
        input({ invoiceLines: [{ itemCode: "VIV10796", quantity: 16 }] }),
      ),
    /remaining open quantity/,
  );
  assert.throws(
    () => buildApInvoiceDraft(input({ invoiceLines: [] })),
